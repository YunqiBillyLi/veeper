    ...
    def updateplot(self):
        ...
        for i, sp in enumerate(self.spls):
            ...
            if ((len(self.fitpars[0]) > 0)):
                ...
                sp.plot(self.wave, self.normflux, drawstyle='steps-mid')
                if self.pixtog == 1:
...
def batch_fit(spec,
    ...
    **kwargs : maxiter, itertol
    ...
    normflux = spectofit.flux.value / spectofit.co
    normsig = spectofit.sig.value / spectofit.co
    cfg.wave = wave
...
def concatenate_all(spectrum, filepath='.', outparfile='.VP'):
    ...
    # os.system("ls " + filepath + "/*.VP > " + filepath + "/all_VP.txt")

    vp_files = glob.glob(filepath + "/*.VP")

    with open(filepath + "/all_VP.txt", "w") as f:
        for file in vp_files:
            f.write(file + "\n")

    with open(filepath + "/all_VP.txt", 'r') as f:
    ...
    jbu.concatenate_line_tables(filepath + "/all_VP.txt")
